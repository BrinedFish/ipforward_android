...
public class UDPTunnel implements KeyHandler {
    ...
    private void processReceived() {
        DebugLog.dWithTag(TAG, "processReceived:" + ipAndPort);
        ...
        try {
            ...
            DebugLog.dWithTag(TAG, "failed to read udp datas ");
            vpnServer.closeUDPConn(this);
            return;
        }
        if (readBytes == -1) {
            vpnServer.closeUDPConn(this);
            DebugLog.dWithTag(TAG, "read  data error :" + ipAndPort);
        } else if (readBytes == 0) {
            DebugLog.dWithTag(TAG, "read no data :" + ipAndPort);
        } else {
    ...
    private void processSend() {
        DebugLog.dWithTag(TAG, "processWriteUDPData " + ipAndPort);
        Packet toNetWorkPacket = getToNetWorkPackets();
        if (toNetWorkPacket == null) {
            DebugLog.dWithTag(TAG, "write data  no packet ");
            return;
        }
        try {
            ...
            DebugLog.wWithLog(TAG, "Network write error: " + ipAndPort, e);
            vpnServer.closeUDPConn(this);
        }
    }

    public void initConnection() {
        DebugLog.dWithTag(TAG, "init  ipAndPort:" + ipAndPort);
    ...
    public void close() {
        try {
            ...
            DebugLog.wWithLog(TAG, "error to close UDP channel IpAndPort" + ipAndPort + ",error is " + e.getMessage());
        }
    ...
    void updateInterests() {
        ...
        DebugLog.dWithTag(TAG, "updateInterests ops:" + ops + ",ip" + ipAndPort);
    }
